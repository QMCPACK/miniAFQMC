...
class NOMSD: public AFQMCInfo
{
  ...
  public:
    ...
    NOMSD(AFQMCInfo& info, afqmc::TaskGroup_& tg_, HamiltonianOperations&& hop_, 
    ...
    // later!!! try returning std::ref instead, since visitor can't figure out that
    // it needs a reference and is making a temporary copy
    std::vector<PsiT_Matrix> const* getOrbMat() const { return &OrbMats; }
/*
    int getOrbSize () { return 2*NMO; }
    std::vector<ComplexType> const& getCiCoeff() { return ci; }
*/
     ...
     */
    template<class WlkSet>
    void Orthogonalize(WlkSet& wset, bool impSamp=true); 
...
#include "Wavefunctions/NOMSD.icc"
