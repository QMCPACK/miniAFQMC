...
int main(int argc, char **argv)
{
  ...
  ComplexMatrix Propg1("Propg1", 1, 1);   // propagator for 1-body hamiltonian 
  ...
  ComplexMatrix vbias("vbias", nchol, nwalk);     // bias potential
  ComplexMatrix vHS("vHS", nwalk, NMO*NMO);        // Hubbard-Stratonovich potential
  ComplexMatrix Gc("Gc", nwalk, NAK);           // compact density matrix for energy evaluation
  ComplexMatrix X("X", nchol, nwalk);         // X(n,nw) = rand(n,nw) ( + vbias(n,nw)) 
  ComplexVector hybridW("hybridW", nwalk);         // stores weight factors
  ComplexVector eloc("eloc", nwalk);         // stores local energies

  WalkerContainer W("W", nwalk, 2, NMO, NAEA);
  ...
  for(int step = 0, step_tot=0; step < nsteps; step++) {
    ...
    for(int substep = 0; substep < nsubsteps; substep++, step_tot++) {
      ...
      for(int i=0; i<hybridW.dimension_0(); i++)
	hybridW(i) = ComplexType(0.);
