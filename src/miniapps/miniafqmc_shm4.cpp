      ...
      if(nnodes_per_TG == 1) {
        ...
        for(int k=0; k<nnodes_per_TG; k++) {
          ...
          if(k < nnodes_per_TG-1) {
            if(node_number%2==0) {
              ma::send(TG.getTGComm(),G_for_vbias[indices[range_t(NIK_0,NIK_N)][range_t()]],TG.getNextRingPattern(),k);
              ma::recv(TG.getTGComm(),Grecv[indices[range_t(NIK_0,NIK_N)][range_t()]],TG.getPrevRingPattern(),k);
            } else {
              ma::recv(TG.getTGComm(),Grecv[indices[range_t(NIK_0,NIK_N)][range_t()]],TG.getPrevRingPattern(),k);
              ma::send(TG.getTGComm(),G_for_vbias[indices[range_t(NIK_0,NIK_N)][range_t()]],TG.getNextRingPattern(),k);
            }
          ...
          TG.local_barrier();
          if(node_number%2==0) {
            ma::send(TG.getTGComm(),vsend[indices[range_t(MM1_0,MM1_N)][range_t()]],TG.getNextRingPattern(),k);
            ma::recv(TG.getTGComm(),vrecv[indices[range_t(MM1_0,MM1_N)][range_t()]],TG.getPrevRingPattern(),k);
          } else {
            ma::recv(TG.getTGComm(),vrecv[indices[range_t(MM1_0,MM1_N)][range_t()]],TG.getPrevRingPattern(),k);
            ma::send(TG.getTGComm(),vsend[indices[range_t(MM1_0,MM1_N)][range_t()]],TG.getNextRingPattern(),k);
          }  
