    ...
    for(int substep = 0; substep < nsubsteps; substep++, step_tot++) {
      ...
      if(nnodes_per_TG == 1) {
        ...
        for(int k=0; k<nnodes_per_TG; k++) {
          ...
          if(node_number==k) {  
            Error! Wrong communicator
            ma::bcast(TG.getTGComm(),G_for_vbias[indices[range_t(NIK_0,NIK_N)][range_t()]],k);
          } else {  
            ma::bcast(TG.getTGComm(),Grecv[indices[range_t(NIK_0,NIK_N)][range_t()]],k);
          }  
          TG.local_barrier();
          ...
          if(transposed_Spvn) {
            ...
            if(node_number==k) 
              ...
              shm::get_vbias(Spvn_for_vbias,Grecv,vbias,transposed_Spvn);
            ...
            if(node_number==k)
              ...
              shm::get_vbias(Spvn_for_vbias,Grecv,loc_vbias,transposed_Spvn);
            // no need for lock, partitionings are non-overlapping 
            vbias[ indices[range_t(Spvn_for_vbias.global_c0(),Spvn_for_vbias.global_cN())][range_t()] ] = 
              loc_vbias[ indices[range_t(Spvn_for_vbias.global_c0(),Spvn_for_vbias.global_cN())][range_t()] ];
          ...
          TG.local_barrier();
          ma::reduce(TG.getTGComm(),vsend[indices[range_t(MM1_0,MM1_N)][range_t()]],k);
