...
template<class T> struct allocator{
	...
	std::reference_wrapper<mpi3::shared_communicator> comm_;
	...
	//	comm_.barrier();
		...
		if(n == 0){
			ret.wSP_ = std::make_shared<shared_window<T>>(
				comm_.get().make_shared_window<T>(0)
			);
			return ret;
		}
		ret.wSP_ = std::make_shared<shared_window<T>>(
			comm_.get().make_shared_window<T>(comm_.get().root()?n:0)
...
*/
	bool operator==(allocator const& other) const{return comm_.get() == other.comm_.get();}
