...
int main(){

	{
		...
		MArray_ref<double, 1> MX(mx.data(), boost::extents[mx.size()]);
		assert( MX == Y );
	}
	{
		std::vector<double> m = {
	...
	MArray_ref<std::complex<double>, 2> M(reinterpret_cast<std::complex<double>*>(m.data()), boost::extents[3][3]);
	assert( ma::is_hermitian(M) );
	}{
	std::vector<double> m = {
		1.,2.,1.,
	...
	MArray_ref<double, 2> AB(ab.data(), boost::extents[3][3]);
	...
	MArray_ref<double, 2> AtB(atb.data(), boost::extents[3][3]);
	assert(C == AtB);
	
	ma::product(A, T(B), C);
	std::vector<double> abt = {14., 14., 10., 92., 92., 110., 112., 121., 141.};
	MArray_ref<double, 2> ABt(abt.data(), boost::extents[3][3]);
	assert(C == ABt);

	ma::product(T(A), T(B), C);
	std::vector<double> atbt = {44., 44., 58., 74., 65., 107., 94., 94., 138.};
	MArray_ref<double, 2> AtBt(atbt.data(), boost::extents[3][3]);
	assert(C == AtBt);
	...
	}
	{
		...
		MArray<double, 2> Id(boost::extents[3][3]);
		ma::set_identity(Id);

		MArray<double, 2> Id2(boost::extents[3][3]);
		ma::product(A, B, Id2);
