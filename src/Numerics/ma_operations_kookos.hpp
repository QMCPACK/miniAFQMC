...
float const& conj(float const& f){return f;}
  ...
  template<class Multi> auto shape(Multi const& m) -> decltype(m.shape()){
    return m.shape();
  }

  template<class MKokkos>
  auto shape_aux(MKokkos const& m, std::integral_constant<1>){
    return std::array<std::size_t, 1>{m.dimension_0()};
  }
  template<class MKokkos>
  auto shape_aux(MKokkos const& m, std::integral_constant<2>){
    return std::array<std::size_t, 2>{m.dimension_0(), m.dimension_1()};
  }
  template<class MKokkos> auto shape(MKokkos const& m){
    return shape_aux(m, std::integral_constant<MKokkos::rank>{});
  }

 
  
template<class Array2D, typename = typename std::enable_if<(Array2D::rank > 1)>::type>
bool is_hermitian(Array2D const& A){
	using std::conj;
	using ma::conj;
	if(share(A)[0] != shape(A)[1]) return false;
	for(int i = 0; i != shape(A)[0]; ++i)
		for(int j = i + 1; j != shape(A)[1]; ++j)
		  if( A(i, j) != conj(A(j, i)) ) return false;
	return true;
}

template<class MultiArray2D, typename = typename std::enable_if<(MultiArray2D::rank > 1)>::type>
bool is_symmetric(MultiArray2D const& A){
	if(shape(A)[0] != shape(A)[1]) return false;
	for(int i = 0; i != A.shape(A)[0]; ++i)
		for(int j = i + 1; j != shape(A)[1]; ++j)
