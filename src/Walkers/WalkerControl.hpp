...
inline int swapWalkersSimple(WlkBucket& wset, Mat&& Wexcess, IVec& CurrNumPerNode, IVec& NewNumPerNode, communicator& comm)
{
  ...
  MyContext = comm.rank();  
  static_assert(std::decay<Mat>::type::dimensionality==2,"Error");
  ...
  for(int ic=0; ic<nswap; ic++)
  {
    ...
    if(minus[ic]==MyContext)
    {
      ...
      wset.push_walkers(MArray_ref<ComplexType,2>(buff.data(),{1,wlk_size}));
    }
  }
  return nswap; 
}
...
inline int swapWalkersAsync(WlkBucket& wset, Mat&& Wexcess, IVec& CurrNumPerNode, IVec& NewNumPerNode, communicator& comm)
{
  ...
  if(deltaN < 0) {
    ...
    {
      requests[ip].wait();
      wset.push_walkers(MArray_ref<ComplexType,2>(buffers[ip],{recvCounts[ip],wlk_size}));
      delete[] buffers[ip];
    }
  } else {
    // sending nodes
