 ...
 */
template<class Vector, class Matrix, class Array>
bool get_nocc_per_kp(Vector const& nmo_per_kp, Matrix const& PsiT, Array&& nocc_per_kp)
{
  ...
  auto find_non_zero = [](auto* ptr, int n, int inc) {
                          for(int p=0; p<n; p++) {
                            if(std::abs(ptr[p*inc]) > 1e-16)
                              return p;
                          }
                          return n;   
                        };
  ...
  int Q = 0;  
  for(int i=0; i<N; i++) {
    auto col = find_non_zero(std::addressof(*PsiT.origin())+i, M, PsiT.stride(0)); 
    ...
    Q = Q_;
    for(int n=bounds[Q+1]; n<M; ++n) {
      if(std::abs(PsiT[n][i]) > 1e-16) {
        std::fill_n(std::addressof(*nocc_per_kp.origin()), nkpts, 0);
        return false;
      }
    } 
...
/*
template<class Array, class Vector, class Matrix>
Array get_PsiK(Vector const& nmo_per_kp, Matrix const& PsiT, int K) 
{
  int nkpts = nmo_per_kp.size();
  int N = PsiT.shape()[0];
  int M = PsiT.shape()[1];
