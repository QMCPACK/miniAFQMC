...
#include "SlaterDeterminantOperations/mixed_density_matrix.hpp"
#include "SlaterDeterminantOperations/apply_expM.hpp"
...
class SlaterDetOperations 
{
  ...
  public:
    ...
    T MixedDensityMatrix(const MatA& hermA, const MatB& B, MatC&& C, bool compact=false) {
      ...
      MArray_ref<T,2> TNN(TMat_NN.data(), {NAEA,NAEA});
      assert(TMat_NM.num_elements() >= NAEA*NMO);
      MArray_ref<T,2> TNM(TMat_NM.data(), {NAEA,NMO});
    ...
    T MixedDensityMatrix(const MatA& hermA, const MatB& B, MatC&& C, communicator& comm, bool compact=false) {
      ...
      MArray_ref<T,2> TNN(SM_TMats->data(), {NAEA,NAEA});
      MArray_ref<T,2> TNM(SM_TMats->data()+NAEA*NAEA, {NAEA,NMO});  
    ...
    void Propagate(Mat&& A, const MatP1& P1, const MatV& V, int order=6) {
      ...
      if(TMat_MN.num_elements() < NMO*NAEA)
        TMat_MN.reextent({NMO,NAEA});
      MArray_ref<T,2> TMN(TMat_MN.data(), {NMO,NAEA});
      MArray_ref<T,2> T1(TMat_NM.data(), {NMO,NAEA});
      MArray_ref<T,2> T2(TMat_MM.data(), {NMO,NAEA});
    ...
    void Propagate(Mat&& A, const MatP1& P1, const MatV& V, communicator& comm, int order=6) {
      ...
      MArray_ref<T,2> T0(SM_TMats->data(), {NMO,NAEA});
      MArray_ref<T,2> T1(SM_TMats->data()+NMO*NAEA, {NMO,NAEA});
      MArray_ref<T,2> T2(SM_TMats->data()+2*NMO*NAEA, {NMO,NAEA});
      if(comm.root()) 
    ...
    T Orthogonalize(Mat&& A) {
      ...
      if(not message) {
        message=true;
        app_warning()<<" WARNING: No gelqf in BGQ. Skipping orthogonalization." <<std::endl;
      }
#else
