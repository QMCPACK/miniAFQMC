...
class THCOps
{
  ...
  using CVector = Kokkos::View<ComplexType*>;
  using CMatrix = Kokkos::View<ComplexType**>;
  using SpCVector = Kokkos::View<SpC*>;
  using SpCMatrix = Kokkos::View<SpC**>;
  ...
  public:
                ...
                E0(e0_)
    {
      ...
      assert(Luv.dimension_0() == Piu.dimension_1());
        // rot Ps are not yet distributed
        assert(rotcPua.dimension_0() == rotPiu.dimension_1());
        if(walker_type==CLOSED)
          assert(rotcPua.dimension_1()==NAEA);
        else if(walker_type==COLLINEAR)
          assert(rotcPua.dimension_1()==NAEA+NAEB);
        else if(walker_type==NONCOLLINEAR)
          assert(rotcPua.dimension_1()==NAEA+NAEB);
        assert(cPua.dimension_0()==Luv.dimension_0());
        if(walker_type==CLOSED)
          assert(cPua.dimension_1()==NAEA);
        else if(walker_type==COLLINEAR)
          assert(cPua.dimension_1()==NAEA+NAEB);
        else if(walker_type==NONCOLLINEAR)
          assert(cPua.dimension_1()==NAEA+NAEB);
      if(walker_type==NONCOLLINEAR) {
        assert(Piu.dimension_0()==2*NMO);
        assert(rotPiu.dimension_0()==2*NMO);
      } else {
        assert(Piu.dimension_0()==NMO);
        assert(rotPiu.dimension_0()==NMO);
      }
      ...
      size_t nu = Luv.dimension_0();
      size_t rnu = rotMuv.dimension_0();
      size_t rnv = rotMuv.dimension_1();
      ...
      Kokkos::resize(TMats, memory_needs);
    }
    ...
    RealType energy(Mat&& E, MatB const& G, bool addH1=true) {
      // G[nel][nmo]
      assert(E.dimension_0() == G.dimension_0());        
      int nwalk = G.dimension_0();
      int nmo_ = rotPiu.dimension_0();
      int nu = rotMuv.dimension_0(); 
      int nv = rotMuv.dimension_1(); 
      int nel_ = rotcPua.dimension_1();
      int nspin = (walker_type==COLLINEAR)?2:1;
      assert(NMO==nmo_);
      assert(G.dimension_1() == nel_*nmo_);        
      using ma::T;
      ...
      std::fill_n(&E(0,0),E.dimension_0()*E.dimension_1(),ComplexType(0.0));
  ...
  protected:
    ...
    void Guv_Guu(MatA const& G, MatB&& Guv, MatC&& Guu, MatD&& T1, bool beta) {
      ...
      int nmo_ = int(rotPiu.dimension_0());
      int nu = int(rotMuv.dimension_0());  // potentially distributed over nodes
      int nv = int(rotMuv.dimension_1());  // not distributed over nodes
      assert(rotPiu.dimension_1() == nv);
      ComplexType zero(0.0,0.0);

      assert(Guu.dimension_0() == nv);
      assert(Guv.dimension_0() == nu);
      assert(Guv.dimension_1() == nv);

      // sync first
      int nel_ = G.dimension_0(); 
      ComplexType scl = (walker_type==CLOSED)?ComplexType(2.0):ComplexType(1.0);
      assert(G.dimension_1() == size_t(nmo_));
      assert(T1.dimension_0() == size_t(nel_));
      assert(T1.dimension_1() == size_t(nv));
